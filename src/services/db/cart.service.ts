...
export default class CartService extends MainDatabaseService {
	...
	public addArray = async (data: Array<
		{
			product_id: number;
			quantity: number;
			user_id: number;
	...
	public async sumOfProducts(data: Array<{
		product_id: number;
		quantity: number;
	}>): Promise<number> {
		try {
			const sums: Array<{ sum: number }> = await Promise.all(data.map((product) => {
				return this.knex('products').where('id', product.product_id)
					.select(this.knex.raw('sum(?? * ??) as ??', ['price', product.quantity, 'sum']))
					.then((res) => res)
					.catch((err) => err);
			}));
			return sums.map((el: { sum: number }) => el[0].sum)
