...
export default class ValidationMiddleware {
  public async registerUserValidation(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
    if (req.body.user) {
      ...
      return res.status(422).json({
        detail: 'Request dont have field "user"',
        message: "validation fails",
      });
    }
  }
  ...
  public cartValidation() {
    return async (req: CartRequest, res: Response, next: NextFunction) => {
      if (req.body.item) {
        ...
        return res
          .status(422)
          .json({ detail: 'Request dont have any of this fields "item"' });
      }
    };
  }
  ...
  public paramIsNumber(params: Array<string>) {
    return (
      ...
      const result = validation
        ...
        : res
            .status(422)
            .json({ detail: `Propery "${errorParam}" must be number` });
      return result;
    };
  }
  ...
  public validateEmail() {
    return async (req: Request, res: Response, next: NextFunction) => {
      if (req.body.email) {
        const validResult = await Validation.emailValidation(req.body.email);
        if (validResult.error) {
          return res.status(422).json({
            detail: validResult.error.details[0],
            message: "validation fails",
        ...
        return res.status(422).json({
          detail: 'Request dont have field "email"',
          message: "validation fails",
        });
      }
    };
