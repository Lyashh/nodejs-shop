...
export default class CartController {
  ...
  public addOrUpdateNotAuth() {
    return (req: CartRequest, res: Response): Promise<Response> | Response => {
      return this.productService
        .findById(req.body.item.product_id)
        .then((product) => {
          /* check if product with id exists */
          if (product) {
            /* found this id in session */
            const found = req.session!.cart.some(
              (el) => el.product_id === req.body.item.product_id
            );
            /* if found -> update */
            if (found) {
              req.session!.cart = req.session!.cart.map(
                (el: { product_id: number; quantity: number }) => {
          ...
          return res.status(404).json({
            detail: `Product with ${req.body.item.product_id} id does not exist`,
          });
        })
        .catch((err) => {
  ...
  public cartByUserId() {
    return (req: CartRequest, res: Response) => {
      return this.cartService
        .cartByUserId(parseInt(req.params.id))
        .then((data) => {
          res.json(data);
        })
